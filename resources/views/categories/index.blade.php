<x-app title="Categories"
    ...
    <x-section title="Categories" :big-title="true" class="xl:max-w-(--breakpoint-lg)">
                ...
                @foreach ($categories as $category)
                    <div>
                        <h2 class="font-bold tracking-widest text-black uppercase text-balance">
                            <a wire:navigate href="{{ route('blog.categories.show', $category) }}"
                                class="underline decoration-1 underline-offset-8 decoration-black/30">
                                {{ $category->name }} →
                            </a>
                        </h2>
                            ...
                            @foreach ($category->activity as $post)
                                <li>
                                    <div class="flex gap-4 items-start md:gap-6">
                                        <a wire:navigate href="{{ route('blog.posts.show', $post) }}"
                                        ...
                                        <div class="leading-normal">
                                            <a wire:navigate href="{{ route('blog.posts.show', $post) }}"
                                                class="font-medium text-balance">
                                                {{ $post->title }}
                                            </a>
                                            ...
                                            <div class="flex flex-wrap gap-x-2 items-center mt-2 text-gray-500 md:mt-1">
                                                ...
                                                <div class="flex items-center">
                                                    <a href="{{ route('blog.posts.show', $post) }}#comments"
                                                        class="underline decoration-gray-600/30 underline-offset-4 decoration-1">
