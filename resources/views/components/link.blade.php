...
<div {{ $attributes }}>
    <a href="{{ $link->url }}" target="_blank">
        @if ($link->image_url)
            <img fetchpriority="high" src="{{ $link->image_url }}" alt="{{ $link->title }}"
        ...
        @else
            ...
            <div
                class="transition-opacity {{ $bgColors }} shadow-md ring-1 ring-black/5 hover:opacity-50 aspect-video rounded-xl shadow-black/5">
            </div>
        @endif
    </a>
    ...
    <div class="flex gap-2 items-center mt-4">
        @if ($link->post)
            <a href="{{ route('posts.show', $link->post) }}"
                class="underline underline-offset-4 decoration-gray-600/30 decoration-1">
        @endif
    ...
    <div class="flex gap-6 justify-between items-center mt-2">
        ...
        <img loading="lazy" src="{{ $link->user->avatar }}" alt="{{ $link->user->name }}"
